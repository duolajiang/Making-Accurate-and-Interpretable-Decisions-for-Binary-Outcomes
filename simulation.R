...
estimation_bart_tree <- function(X,true.names.Z1,true.value.Z1,
                                                      ...
                                                      scenario,lambda,ntree,
                                                      complexity,n_sample,n_sim){
  y1.mean.true <- matrix(0,nrow=n_sim,ncol=1000)
  y0.mean.true <- matrix(0,nrow=n_sim,ncol=1000)
  y1.mean.m1 <- matrix(0,nrow=n_sim,ncol=1000)
  y0.mean.m1 <- matrix(0,nrow=n_sim,ncol=1000)
  ite.m2 <- array(rep(0, complexity*1000*n_sim), c(complexity,1000,n_sim))
  y1.mean.m3 <- array(rep(0, complexity*1000*n_sim), c(complexity,1000,n_sim))
  y0.mean.m3 <- array(rep(0, complexity*1000*n_sim), c(complexity,1000,n_sim))
  ...
  trt <- rbinom(nrow(X),1,pr.trt)
  observe.y <- trt*pr.y1$Y + (1-trt)*pr.y0$Y
  population <- cbind(X,trt,observe.y,pr.y1$pr,pr.y0$pr)
  colnames(population) <- c(colnames(X),"trt","observe.y","pr.y1","pr.y0")
  rownames(population) <- seq(dim(population)[1])
  
  for (i in seq(n_sim)){
    sample.id <- sample(rownames(population),n_sample)
    data <- population[sample.id,]
    y1.mean.true[i,] <- data$pr.y1
    y0.mean.true[i,] <- data$pr.y0
    X <- subset(data, select = -c(observe.y,trt,pr.y1,pr.y0))
    rownames(X) <- seq(1,dim(X)[1],1)
    Z <- data$trt
    y <- data$observe.y
    m <- dim(X)[1]
    ...
    y1.mean <- apply(post$prob.test[,1:m],2,mean); y1.var <- apply(post$prob.test[,1:m],2,var)
    y0.mean <- apply(post$prob.test[,(m+1):(2*m)],2,mean); y0.var <- apply(post$prob.test[,(m+1):(2*m)],2,var)
    y1.mean.m1[i, ] <- y1.mean
    y0.mean.m1[i, ] <- y0.mean
    
    test <- subset(data, select = -c(observe.y,pr.y1,pr.y0))
    test1 <- test0 <- test; test1$trt <- 1; test0$trt <- 0; test <- rbind(test1,test0)
    
    m2.train <- X
    m2.train$y <- y1.mean - y0.mean
    m2.train$predictive.var <- apply(post$prob.test[,1:m]-post$prob.test[,(m+1):(2*m)],2,var)
    m3.train <- X; m3.train$trt <- Z; m3.train$y <- y; m3.train$predictive.var <- 0
    
    for (depth in seq(1,complexity,1)){
      tree <- grow.INT.ILL(dat=m2.train,
                           split.var=1:12,ctg=c(1:10),
                           min.ndsz=5, pt=0, max.depth=depth,alpha=0,
                           mtry=12,loc=FALSE)
      ite <- predict_initree_ITE(tree,m2.train,ctg=c(1:10))
      ite.m2[depth, ,i] <- ite
      ...
      outcome <- predict_initree_ITE(tree,test,ctg=c(1:10,13))
      y1.m3 <- outcome[1:m]
      y0.m3 <- outcome[(m+1):(2*m)]
      y1.mean.m3[depth, ,i] <- y1.m3
      y0.mean.m3[depth, ,i] <- y0.m3
    }
  }
  
  return(list(y1.mean.true, y0.mean.true,
              y1.mean.m1,   y0.mean.m1, 
              ite.m2,
              y1.mean.m3,   y0.mean.m3))
}

# '@ return a list (y1.mean.true, y0.mean.true, m1.d, m2.d, m3.d.y1, m3.d.y0)
# '@ y1.mean.true: a dataframe with n_sim rows and n_sample columns. 
#    The dataframe is the probability of p(Y(1)=1) for each individual, 
# '@ y0.mean.true: a dataframe with n_sim rows and n_sample columns. 
#    The dataframe is the probability of p(Y(0)=1) for each individual, 
# '@ m1.d: a dataframe with n_sim rows and n_sample columns. The dataframe is 
#    the probability of p(z*=1) given loss functions, where potential outcomes are estimated using m1.
# '@ m2.d: a dataframe with length(seq(0,alpha,step)) rows and n_sample columns and 
#    n_sim third dimension. The dataframe wi the probability of p(z*=1) summarized from p(z*=1) estimated from m1. 
# '@ m3.d.y1, m3.d.y0 is a dataframe with length(seq(0,alpha,step)) rows and 
#    n_sample columns and n_sim third dimension, the dataframe is probability 
#    of p(Y(1)=1) and p(Y(0)=1) modeled using m3. 
estimation_bart_lreg <- function(X,true.names.Z1,true.value.Z1,true.names.Z0,true.value.Z0,scenario,lambda,ntree,alpha,step,epoch,eta,batch,n_sample,n_sim,L2=FALSE,ct00, ct01,ct10,ct11,cc00, cc01,cc10,cc11,rho){
  #browser()
  if((scenario=='F')|(scenario=='G')|(scenario=='H')){
    pr.y1 <- generate(X, true.names.Z1, true.value.Z1,paste(scenario,'1',sep = ''))
    pr.y0 <- generate(X, true.names.Z0, true.value.Z0,paste(scenario,'0',sep = ''))
  ...
  m1.d <- array(rep(0,n_sim*n_sample),c(n_sim,n_sample))
  m2.d <- m3.d.y1 <- m3.d.y0 <- array(rep(0, length(seq(0,alpha,step))*n_sample*n_sim), c(length(seq(0,alpha,step)),n_sample,n_sim))
  y1.mean.true <- matrix(0,nrow=n_sim,ncol=n_sample)
  y0.mean.true <- matrix(0,nrow=n_sim,ncol=n_sample
                         )
  ...
  for (i in seq(n_sim)){
    ...
    ntree <- ntree
    train <- cbind(X,Z)
    test1 <- test0 <- subset(data, select = -c(observe.y,trt,pr.y1,pr.y0))
    test1$Z <- 1
    test0$Z <- 0
    test <- rbind(test1,test0)
    post <- pbart(train,y,test,ntree = ntree)
    post$prob.test <- pnorm(post$yhat.test)
    y1 <- post$prob.test[,1:m]
    y0 <- post$prob.test[,(m+1):(2*m)]
    p11 <- rho*sqrt(y1*(1-y1)*y0*(1-y0))+y1*y0
    p10 <- y1 - p11
    p01 <- y0 - p11
    p00 <- 1 - p11 - p10 - p01
    L1 <- ct00*p00 + ct01*p01 + ct10*p10 + ct11*p11
    L0 <- cc00*p00 + cc01*p01 + cc10*p10 + cc11*p11
    pz <- apply(((L1 - L0)< 0)*1,2,mean)
    m1.d[i,] <- pz
    
    # model.matrix generate the first column as constant 1
    m2.X <- train
    m2.X <- model.matrix(~ a+b+c+d+e+A+B+C+D+E+Ca+Cb,data = m2.X)
    m2.X <- as.matrix(m2.X)
    m2.y <- pz
    ...
    test.X.m2 <- subset(data, select = -c(observe.y,trt,pr.y1,pr.y0))
    test.X.m2 <- model.matrix(~ a+b+c+d+e+A+B+C+D+E+Ca+Cb,data = test.X.m2)
    
    test.X <- subset(data, select = -c(observe.y,trt,pr.y1,pr.y0))
    test.X.1 <- test.X.0 <- test.X
    test.X.1$Z <- 1; test.X.0$Z <- 0;   
    test.X.1 <- model.matrix(~ a+b+c+d+e+A+B+C+D+E+Ca+Cb+Z+Z:a+Z:b+Z:c+Z:d+Z:e+Z:A+Z:B+Z:C+Z:D+Z:E+Z:Ca+Z:Cb,data = test.X.1)
    test.X.0 <- model.matrix(~ a+b+c+d+e+A+B+C+D+E+Ca+Cb+Z+Z:a+Z:b+Z:c+Z:d+Z:e+Z:A+Z:B+Z:C+Z:D+Z:E+Z:Ca+Z:Cb,data = test.X.0)
    ...
    for (ralpha in regularization){
      m2.weight <- LRegSGD(X=m2.X,Y=m2.y,eta=eta,alpha=ralpha,batch=batch,epoch=epoch)
      m2.Y1.hat.logit <- test.X.m2%*%m2.weight
      m2.Y1.hat <- 1/(1+exp(-m2.Y1.hat.logit))
      m2.d[j,,i] <- m2.Y1.hat
      ...
      m3.d.y1[j,,i] <- m3.Y1.hat
      m3.d.y0[j,,i] <- m3.Y0.hat
      
      j <- j+1
    }
  }
  
  return(list(y1.mean.true, y0.mean.true, m1.d, m2.d, m3.d.y1, m3.d.y0))
}


SimulationMain_LReg <- function(scenario = 'A',
                                true.names.Z1 = c("C1", "B1", "a3:A1"),
                                true.value.Z1 =  c(0.5, 2, 2),
                                true.names.Z0 =  c("C1"),
                                true.value.Z0 =  c(0.5),
  ...
  scenario.m1 <-  scenario.Bias <- scenario.Variance <- scenario.MSE <- NULL
  outcome <- FUN(X,true.names.Z1,true.value.Z1,true.names.Z0,true.value.Z0,scenario,lambda,ntree,alpha,step,epoch,eta,batch,n_sample,n_sim,L2,ct00, ct01,ct10,ct11,cc00, cc01,cc10,cc11,rho)
  filename <- paste(file_name,scenario,'.RData',sep = '')
  save(outcome,file = filename)
}
